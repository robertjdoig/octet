...
namespace octet {
  ...
  public:
    ...
    float rotAngle = 45;
    btVector3 ballForce = btVector3(100, 0, 0);
    ...
    void app_init() {
      ...
      //draw bridge
      drawBridge(mat, vec3(0, 5, 0), rotAngle, 0, 0, 1, vec3(12, 0.1f, 5), vec4(0.2f, 1, 0.2f, 1), false);
    ...
    void drawCylinder(mat4t _mat, vec3 _pos, vec3 _size, vec4 _col, bool isMovable) {
      ...
      material *locMat = new material(_col);
      app_scene->add_shape(_mat, new mesh_cylinder(_size,1), locMat, isMovable);
